...
export class FileQueueRepo {
    ...
    constructor(@Inject('PG_BOSS') private readonly boss: PgBoss) {
        this.queueReady = new Promise((resolve, reject) => {
            this.boss.getQueue(this.queueName).then((queue) => {
                if (queue === null) {
                    this.boss.createQueue(this.queueName, { policy: 'singleton', name: this.queueName }).then(() => {
                        resolve();
                    }).catch((err) => {
                        reject(err);
                    });
    ...
    async dequeue( fileId: string): Promise<void> {
        await this.queueReady;
        const job: JobWithMetadata<FileJob> | null = await this.boss.getJobById(this.queueName, fileId);
