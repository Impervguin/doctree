...
export class FileInfoRepository {
    ...
    async save(fileInfo: StoredFileInfo): Promise<void> {
        let repo = this.dataSource.getRepository(FileInfo);

        return new Promise((resolve, reject) => {
            repo.save(FileInfoMapper.toEntity(fileInfo)).then(_ => resolve()).catch(reject);
        });
    }
    ...
    async hardDelete(fileId: string): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            repo.delete({ id: fileId }).then(_ => resolve()).catch(reject);
        });
    }
    ...
    async get(fileId: string): Promise<StoredFileInfo | null> {
        ...
        return repo.findOneBy({ id: fileId }).then(entity => entity ? FileInfoMapper.toDomain(entity) : null);
    }
    ...
    async softDelete(fileId: string): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            repo.softDelete({ id: fileId }).then(_ => resolve()).catch(err => reject(err));
        });
    }
