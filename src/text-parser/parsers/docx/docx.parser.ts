...
@Injectable()
@RegisterParser('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
export class DocxParser implements Parser {
    ...
    async parse(file: Buffer): Promise<ParserResponse> {
        let files =await this._unzip(file, DocxParser.filesToUnzip);
        let filenames = files.map(file => file.name);

        let footnotesFile = files.find(file => file.name === DocxParser.footnotesPath);
        let endnotesFile = files.find(file => file.name === DocxParser.endnotesPath);
        let documentFile = files.find(file => file.name === DocxParser.documentPath);
        if (!documentFile) {
            return {
                text: '',
                parsePercentage: 0,
    ...
    async _unzip(file: Buffer, filesToUnzip: string[] = []): Promise<UnzippedFile[]> {
        return new Promise((resolve, reject) => {
            const unzippedFiles: UnzippedFile[] = [];
            const zip = new fromBuffer(file, { lazyEntries: true }, (err, zipfile) => {
                if (err) {
                    reject(err);
                }
                zipfile.readEntry();
    ...
    async _parseDocument(docFile: Buffer, footnotesFile?: Buffer, endnotesFile?: Buffer): Promise<string> {
