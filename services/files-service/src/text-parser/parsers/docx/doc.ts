...
export class Doc {
    ...
    private static readonly paragraphParserOptions = {
        ...
        isArray: (name, jpath, isLeafNode, isAttribute) => name === 'w:r' || name === 'w:p',
    }

    private static readonly footnoteParserOptions = {
        ignoreAttributes: false,
        attributeNamePrefix: Doc.attributeNamePrefix,
        isArray: (name, jpath, isLeafNode, isAttribute) => name === 'w:r' || name === 'w:p' || name === 'w:footnote',
    }

    private static readonly endnoteParserOptions = {
        ignoreAttributes: false,
        attributeNamePrefix: Doc.attributeNamePrefix,
        isArray: (name, jpath, isLeafNode, isAttribute) => name === 'w:r' || name === 'w:p' || name === 'w:endnote',
    }
    ...
    async _getFootnote(id : number): Promise<string> {
        ...
        const footnoteObj = this.footnotes['w:footnotes']['w:footnote'].find(footnote => footnote[Doc._attribute('w:id')] === `${id}`);
        if (!footnoteObj) {
            throw new Error(`Footnote with id ${id} not found`);
        }

        let footnoteStrings: Promise<string>[] = [];
    ...
    async _getEndnote(id: number): Promise<string> {
        ...
        const endnoteObj = this.endnotes['w:endnotes']['w:endnote'].find(endnote => endnote[Doc._attribute('w:id')] === `${id}`);
        if (!endnoteObj) {
            throw new Error(`Endnote with id ${id} not found`);
        }

        let endnoteStrings: Promise<string>[] = [];
    ...
    async _parseParagraph(paragraph: any): Promise<string> {
        let paragraphStrings: Promise<string>[] = [];
        if (!paragraph['w:r']) {
            return '';
        }
        paragraph['w:r'].forEach((runObj: any) => {
            ...
            // Reference of footnote
                ...
                const footnoteId : number = parseInt(runObj['w:footnoteReference'][Doc._attribute('w:id')]);
                paragraphStrings.push(this._getFootnote(footnoteId).then(Doc._footnoteFormat));
            // Reference of endnote
            } else if (runObj['w:endnoteReference']) {
                const endnoteId : number = parseInt(runObj['w:endnoteReference'][Doc._attribute('w:id')]);
                paragraphStrings.push(this._getEndnote(endnoteId).then(Doc._endnoteFormat));
            }
        });
