...
import { NotFoundError, ConflictException } from '@doctree/shared/errors/errors'
  ...
  private async checkForLoop(nodeId: string, potentialParentId: string): Promise<void> {
    ...
    while (currentParentId) {
      if (visited.has(currentParentId)) {
        throw new ConflictException('This change would create a circular dependency in the hierarchy');
      }
